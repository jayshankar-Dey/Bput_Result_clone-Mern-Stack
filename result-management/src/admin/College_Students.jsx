...
const College_Students = () => {
  ...
  const getSingleStudent=async(id)=>{
    const res = await axios.get(
      `http://localhost:8080/api/v1/student/all?id=${id}`,
      {
        headers: {
...
const handlesubmit = async (e) => {
  e.preventDefault();
  //setloading(true);
  const res = await axios.put(`http://localhost:8080/api/v1/student/profile/update/${ID}`, {
...
const DeleteStudent=async(id)=>{
  const res = await axios.delete(`http://localhost:8080/api/v1/student/delete/${id}`, 
  {
    headers:{
      Authorization: `Bearer ${localStorage.getItem("Bputoken")}`
  }
...
const getAllCollage =async()=>{
  const res = await axios.get(`http://localhost:8080/api/v1/collage/all/${search}`,{
      headers:{
          Authorization: `Bearer ${localStorage.getItem("Bputoken")}`,
      }
  })
 setColleges(res.data.collage)
  ...
  const get_all_students = async () => {
    ...
    const res = await axios.get(
      `http://localhost:8080/api/v1/student/all?collage=${id}&page=${page}&reg=${searchReg}`,
      {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("Bputoken")}`,
        },
