...
const handlesubmit = async (e) => {
  e.preventDefault();
  //setloading(true);
  const res = await axios.post(`http://localhost:8080/api/v1/student/add`, {
    collage, studentName, branch, reg, address
  },
  {
    headers:{
...
const get_all_students = async () => {
  ...
  //setloading(true);
  const res = await axios.get(`http://localhost:8080/api/v1/student/all?reg=${Student_search}&page=${page}`,
  {
    headers:{
      Authorization: `Bearer ${localStorage.getItem("Bputoken")}`,
  }
