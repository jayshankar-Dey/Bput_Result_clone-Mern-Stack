...
const SingleStudent = () => {
 ...
 const add_student_semistar=async(e)=>{
  ...
  const res = await axios.put(
    `http://localhost:8080/api/v1/student/semistar/create/${id}`,{ semister, type, year },
    {
      headers: {
        Authorization: `Bearer ${localStorage.getItem("Bputoken")}`,
      },
...
const setProfileImage=async()=>{
  ...
  setloading(true)
  const res = await axios.put(`http://localhost:8080/api/v1/student/profileImage/update/${id}`, formdata,
  {
    headers:{
      Authorization: `Bearer ${localStorage.getItem("Bputoken")}`
  }
