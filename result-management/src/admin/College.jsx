...
const College = () => {
   ...
   const[change, setChange] = useState("")
    ...
    const AddCollege=async(e)=>{
        e.preventDefault()
        setLoading(true)
        const res = await axios.post(`http://localhost:8080/api/v1/collage/add`, {collageName, address},{
            headers:{
...
const getAllCollage =async()=>{
    const res = await axios.get(`http://localhost:8080/api/v1/collage/all/${search}?page=${page}`,{
        headers:{
            Authorization: `Bearer ${localStorage.getItem("Bputoken")}`,
        }
    })
