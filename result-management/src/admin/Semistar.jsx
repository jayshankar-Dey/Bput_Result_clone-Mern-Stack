...
const Semistar = ({exam,change,id}) => {
  ...
  const DeleteStudent_semistar=async(exam_id)=>{
    ...
    const res = await axios.delete(`http://localhost:8080/api/v1/student/delete/semistar/${id}?exam_id=${exam_id}`, 
      {
        headers:{
  ...
  const Update_student_semistar=async(e)=>{
    e.preventDefault()
    const res = await axios.put(`http://localhost:8080/api/v1/student/update/${id}?exam_id=${exam._id}`, {semister,type,year},
      {
        headers:{
          Authorization: `Bearer ${localStorage.getItem("Bputoken")}`
      }
  ...
  const HandleupdateMark=async(e)=>{
    e.preventDefault()
    const res = await axios.post(`http://localhost:8080/api/v1/student/add/mark`, {subjectCode,subjectName,type:Type,cradit,finamGrade,examId},
      {
        headers:{
          Authorization: `Bearer ${localStorage.getItem("Bputoken")}`
      }
