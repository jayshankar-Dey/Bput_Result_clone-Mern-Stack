...
const CollegeTableRow = ({collage,change}) => {
    ...
    const deleteCollege=async(id)=>{
      const res = await axios.delete(`http://localhost:8080/api/v1/collage/delete/${id}`,{
          headers:{
    ...
    const getSingleCollege=async(id)=>{
      const res = await axios.get(`http://localhost:8080/api/v1/collage/get/${id}`,{
          headers:{
              Authorization: `Bearer ${localStorage.getItem("Bputoken")}`,
          }
      })
  ...
  const UpdateCollege=async(e)=>{
    e.preventDefault()
    setloading(true)
    const res = await axios.put(`http://localhost:8080/api/v1/collage/update/${ID}`,{collageName,address},{
      headers:{
        Authorization: `Bearer ${localStorage.getItem("Bputoken")}`,
    }
    })
